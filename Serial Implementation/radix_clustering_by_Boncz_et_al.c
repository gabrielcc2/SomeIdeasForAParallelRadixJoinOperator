...
/* target input: prog file
And the file would have:
First line: <MHz> 
Second line: (<pass_bits>). 
Other n lines: elements for tuples (their position is inplicit)

Implicitly, with the number of elements in the file, we get the number of tuples.
With the sum of the pass_bits we get the number of passes, and the number of bits considered.* */
...
int main(uint argc, char* argv[]) { //NOTE: This algorithm does a most significant bit first clustering.
	...
	printf("%d %d cycles\n", n, (timer()-t)/10);
	return 0;//fetched[0] + result0[0].pos;  /* dummy value to force intelligent compilers to do the work */
}
