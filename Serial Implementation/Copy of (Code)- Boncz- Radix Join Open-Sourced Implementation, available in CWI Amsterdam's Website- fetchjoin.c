...
void print(uint v, uint* fetch, tuple* input, tuple* result0, tuple* result1, uint* fetched) {
	uint i;
	printf("          +--------------------------------+---------------------+---------------------+----------+\n");
	printf("          +   fetch* |               input | after radix-cluster |    after fetchjoin2 |          |\n");
	printf("          +----------+----------+----------+----------+----------+----------+----------+----------+\n");
	printf("          |   fetch* |      key |      pos |      key |      pos |      key |      pos | fetched* |\n");
	printf("          +----------+----------+----------+----------+----------+----------+----------+----------+\n");
	for(i=0; i<v; i++)
		printf(" % 8u | % 8u | % 8u | % 8u | % 8u | % 8u | % 8u | % 8u | % 8u |\n", i, 
			fetch[i], input[i].key, input[i].pos, result0[i].key, result0[i].pos, result1[i].key, result1[i].pos, fetched[i]);
	printf("          +----------+----------+----------+----------+----------+----------+----------+----------+\n");
}
...
int main(uint argc, char* argv[]) {
	uint b = atoi(argv[1]); /* total significant bits (ie datasize) */
	uint n = 1 << b; /* number of tuples, given as log2 */
	uint r = (100000LL * atoi(argv[2])) / n; /* repeats. idea: r*n = Hz of the machine */
	uint w = 1 << atoi(argv[3]); /* decluster window (0=run cluster-only) */
	uint v = atoi(argv[4]); /* verbose output of up to v tuples */
	uint c = atoi(argv[5]); /* verbose output of up to v tuples */
	uint *nbits = (uint*) alloca(argc*sizeof(uint)); /* an array, because eas pass can have a different number of bits */
	uint *fetch = (uint*) malloc(c*n*sizeof(uint));
	uint *fetched = (uint*) malloc(c*n*sizeof(uint));
	tuple *result0 = (tuple*) malloc(n*sizeof(tuple));
	tuple *result1 = (tuple*) malloc(n*sizeof(tuple));
	tuple *input = permute(n);
	...
	if (v) print((v > n)?n:v, fetch, input, result0, result1, fetched);
	printf("%d %d cycles\n", n, (timer()-t)/10);
